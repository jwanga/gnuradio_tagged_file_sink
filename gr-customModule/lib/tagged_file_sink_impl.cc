...
{
    return gnuradio::make_block_sptr<tagged_file_sink_impl>(itemsize, samp_rate, filename);
}
        ...
        if (d_state == state_t::NOT_IN_BURST) {
            while (vitr != all_tags.end()) {
                if ((pmt::eqv((*vitr).key, bkey)) && pmt::is_true((*vitr).value)) {
                    ...
                    // If the user has passed in a filename pattern, use it.
                    if (d_filename != NULL) {

                      if(!strstr(d_filename, "{index}") && !strstr(d_filename, "{time}")) {
                        d_logger->error("You must include \"{index}\" or \"{time}\" in your filename in order for each file to be unique. {:s}", strerror(errno));
                      }

                      filename = fmt::format(
                          d_filename, fmt::arg("id", unique_id()), fmt::arg("index", d_n), fmt::arg("time", d_timeval));
                    } else {
                      filename = fmt::format(
